...
<div class="frameR"><div class="content">
	...
	<div class="loc icon"><samp class="t12"></samp>当前位置：权限管理&nbsp;&raquo;&nbsp;<a href="<c:url value="/${system }/role/listRole.do"/>" title="用户管理">用户管理</a>&nbsp;&raquo;&nbsp;<span class="gray">编辑用户</span>
    <a href="<c:url value="listRole.do"/>" title="返回用户管理" class="inb btn80x20">返回用户管理</a>
    </div>
	<form id="form111" name="form111" action="${path}/role/editSaveRole.do" method="post" enctype="multipart/form-data">
		<div class="edit set">
		<input type="hidden" id="roleId" name="roleId" value="${roleOne.roleId}"/>
		<input type="hidden" id="createTime" name="createTime" value="${roleOne.createTime}"/>	
			<p>
			    <label><samp>*</samp>角色名:</label>
			    <input type="text" id="roleName" name="roleName" class="text state" value="${roleOne.roleName}" reg2="^(.|\n){0,300}$" tip="任意字符，长度0-300"/>
				<span></span>
			</p>
			<p>
			    <label><samp>*</samp>角色描述：</label>
			    <input type="text" id="roleAddress" name="roleAddress" class="text state" value="${roleOne.roleAddress}"  reg2="^(.|\n){0,300}$" tip="任意字符，长度0-300"/>
				<span></span>
			</p>
			<p>
			    <label><samp>*</samp>所属学校：</label>
					...
					<c:forEach items="${schoolListById}" var="school">
